...
export default function OrderDetail() {
    ...
    const setStates = (res) => {
        ...
        setOrderInfo({
            ...
            claimStatusTypeLabel:
                orderOptionsGroupByPartner[0].orderOptionsGroupByDelivery[0]
                    .orderOptions[0].claimStatusTypeLabel,
            retrieveInvoiceUrl:
                orderOptionsGroupByPartner[0].orderOptionsGroupByDelivery[0]
                    .retrieveInvoiceUrl,
            extraData,
        ...
        setClaimInfo(() => ({
            claimStatusType:
                orderOptionsGroupByPartner[0].orderOptionsGroupByDelivery[0]
                    .orderOptions[0].claimStatusType,
            claimNo:
                orderOptionsGroupByPartner[0].orderOptionsGroupByDelivery[0]
                    .orderOptions[0].claimNo,
        }));
        ...
        const promotionDiscountAmt =
            immediateDiscountAmt + additionalDiscountAmt;
    ...
    const showOrderConfirm = (orderStatusType) =>
        'DELIVERY_DONE' === orderStatusType;
    ...
    const isClaimStart = (defaultOrderStatusType) => {
        ...
        return claimStatuses.some((claimStatus) =>
            defaultOrderStatusType.includes(claimStatus),
        );
    };
    ...
    const _cancelOrder = (bankAccountInfo = null) => {
        const isAllSameOrerSatatus = orderProducts.every(
            (orderProduct, _, orderProducts) =>
                orderProducts[0].orderStatusType ===
                orderProduct.orderStatusType,
        );
        if (!isAllSameOrerSatatus) {
            alert(
                '주문상태가 다른 상품이 있습니다. 소니 고객센터로 연락바랍니다.',
            );
            return;
        }
        ...
        const isDeliveryPrepare = [
            'PRODUCT_PREPARE',
            'DELIVERY_PREPARE',
        ].includes(orderProducts[0].orderStatusType);
        const postCancel = isDeliveryPrepare
            ? orderOptionCancelMap
            : orderCancelMap;

        if (isDeliveryPrepare) {
            request.requestBody.claimedProductOptions = orderProducts.map(
                ({ orderOptionNo, orderCnt }) => ({
                ...
                if (payInfo.payType === 'ESCROW_VIRTUAL_ACCOUNT' || payInfo.payType === 'VIRTUAL_ACCOUNT') {
                    return async () => {
                        ...
                        const { claimStatusType, claimNo } =
                            data.orderOptionsGroupByPartner[0]
                                .orderOptionsGroupByDelivery[0].orderOptions[0];
    ...
    return (
        <>
            ...
            {confirm.visible && (
                <Confirm onClose={onCloseConfirm}>{confirm.message}</Confirm>
            )}
                ...
                <div className='container my'>
                    <div className='content'>
                        ...
                        {!isClaimStart(orderInfo.defaultOrderStatusType) && (
                            <OrderProcess
                                defaultOrderStatusType={
                                    orderInfo.defaultOrderStatusType
                                }
                            />
                        )}
            ...
            {refundAccountVisible && (
                <RefundAccount
                    setVisible={setRefundAccountVisible}
