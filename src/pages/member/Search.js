...
export default function Search() {
    ...
    const searchMember = async () => {
        ...
        if (tabState === 'id') {
            if (!validationSearch(searchValue.mobileNo, searchValue.memberName))
                return;
            ...
            if (!validationSearch(searchValue.mobileNo, searchValue.email))
                return;
            const { data: response } = await getMemberInfo({
                type: '20',
            ...
            if (response.errorCode === '0000') {
                openAlert(
                    `${response.body.customerid} 메일을 보냈습니다.`,
                    true,
                );
            } else {
                openAlert(response.errorMessage);
