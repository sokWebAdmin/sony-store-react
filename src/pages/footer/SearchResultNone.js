...
import { RECOMMEND_SECTION } from '../../const/search';
  ...
  const fetchRecommendedProducts = async () => {
    try {
      const { data } = await getDisplaySectionsSectionNo({
        pathParams: {
          sectionNo: RECOMMEND_SECTION.no,
        },
        params: {
          by: 'ADMIN_SETTING',
          soldout: true,
      ...
      <div className="guidesearch">
        <ul className="guidesearch__list">
          ...
          <li>
            <em className="icon"></em>
            <strong>
              정확한 모델명을 <span>모르는 경우</span>
            </strong>
            <p>
              DSC-RX100M3의 경우
              <br />
              DSC*로 상품명 뒤에 *를 넣어서 검색해주세요.
            </p>
            <span className="desc">
              *인기 검색어를 활용하셔도
              <br />
              소니 제품을 쉽게 찾으실 수 있습니다.
            </span>
          </li>
        </ul>
      </div>
