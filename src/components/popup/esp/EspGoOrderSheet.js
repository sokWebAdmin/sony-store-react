...
export default function EspGoOrderSheet({ product, onClose, history }) {
  ...
  const _getOrderSheetNo = async (productNo, optionNo) => {
    try {
  ...
  const _fetchOrderSheetNo = async () => {

    const pathname = '/order/sheet';
    ...
    if (options.length > 0) {
      try {
        const result = await _getOrderSheetNo(query.productNo, options[0].optionNo);
        if (result?.code) {
    ...
    <>
      <LayerPopup className="esp_pop" size={'m'} popContClassName={'scrollH'} onClose={onClose}>
        ...
        <div className="agree_check_box">
          <div className="check">
            <input type="checkbox" className="inp_check" id="agree_check" onChange={_fetchOrderSheetNo} />
            <label htmlFor="agree_check">동의합니다.</label>
          </div>
          <p>
            '연장 서비스플랜 이용약관' 동의 시<span className="mo"> 주문·결제 페이지로 이동합니다.</span>
          </p>
        </div>
      </LayerPopup>
