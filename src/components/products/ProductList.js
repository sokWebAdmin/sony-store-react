...
export default function ProductList({ category }) {
    ...
    const _getProducts = async () => {
        ...
        try {
            ...
            result.list = await _getGroupManagementMappingProducts(
                data.items.map(({ productName, ...rest }) => ({
                    ...rest,
    ...
    const _getGroupManagementMappingProducts = async (list) => {
        ...
        try {
            let arrExistGroup = result
                .filter((p) => !!p.groupManagementCode)
                .map((p) => p.groupManagementCode);
            ...
            if (arrExistGroup.length > 0) {
                ...
                data.map((g) => {
                    result
                        .filter(
                            (p) =>
                                p.groupManagementCode === g.groupManagementCode,
                        )
                        .map((p) => {
                            p.groupManagementMappingProducts =
                                g.groupManagementMappingProducts.sort((gp) =>
    ...
    return (
        <div className='product__list__wrapper'>
            ...
            <div
                className={`itemsort ${orderOpen ? 'itemsort--open' : ''}`}
                aria-label='상품 정렬'
                ref={productsRef}
            >
                <button
                ...
                >
                    ...
                    <span className='itemsort__button__selected'>
                        {orderList[currentOrder.index].title}
                    </span>
                </button>
