...
const CategoryHeader = ({ category, changeCurrentCategoryByNo }) => {
    ...
    const openLink = ({
        isAvailableMoveProductCompare,
        isAvailableMoveAccessoryCompatibility,
        e,
    }) => {
        ...
        if (isAvailableMoveAccessoryCompatibility) {
            window.openWindow(
                'https://support.d-imaging.sony.co.jp/www/cscs/accessories/top.php?area=ap&lang=ko',
                '_blank',
            );
        }
    ...
    const firstDepthCategoryHandler = () => {
        ...
        const list =
            firstDepthCategoryRef.current.querySelectorAll('.swiper-slide');
        ...
        const headerWidth = parseInt(
            getComputedStyle(categoryHeaderRef.current).width,
        );
        const innerWrapper =
            firstDepthCategoryRef.current.querySelector('.swiper-wrapper');
    ...
    return (
        <div
            ...
            className={
                category?.depth > 1
                    ? 'category__header category__header__sub'
                    : 'category__header '
            }
        ...
        >
            {category?.parent && (
                <a
                    href='#'
                    className='category__header__back'
                    onClick={(e) => {
                ...
                <div className='category__header__menu swiper-container'>
                    ...
                    >
                        {category?.children.length > 0 && (
                            <SwiperSlide
                                key='all'
                                className='swiper-slide all category__header__menu--active'
                            >
                                <a>
                                    <span>전체보기</span>
                                </a>
                        ...
                        {category?.depth === 1 &&
                            category?.children.map((c) => {
                                return (
                                    <SwiperSlide
                                        onClick={() => history.push(c.url)}
                                        className='swiper-slide'
                                        style={{
                                            backgroundImage: `url(${c.icon})`,
                                        }}
                                        key={`sub-category-${c.categoryNo}`}
                                    >
                                        <Link to={c.url}>
                                            <span>{c.label}</span>
                                        </Link>
            ...
            {rootParent && (
                <div className='category__header__links'>
                    {rootParent.isAvailableMoveProductCompare && (
                        <a
                            href='#'
                            className='category__header__link'
                            onClick={(e) => {
                                openLink({
                    ...
                    {rootParent.isAvailableMoveAccessoryCompatibility &&
                        categoryLabel !== 'audio' && (
                            <a
                                href='#'
                                className='category__header__link'
                    ...
                    {rootParent.isAvailableMoveESP && (
                        <Link to='/esp' className='category__header__link'>
                            연장 서비스 플랜 ESP 보기
                        </Link>
                    )}
                </div>
            ...
            {category?.depth > 1 && (
                <div className='category__header__menu swiper-container'>
                    ...
                    {category?.children.length > 0 && (
                        ...
                        >
                            <SwiperSlide
                                className={`swiper-slide all ${
                                    currentCategoryNo === category.categoryNo
                                        ? 'category__header__menu--active'
                                        : ''
                                }`}
                            >
                                <a
                            ...
                            {category?.children.map((c) => {
                                return (
                                    <SwiperSlide
                                        className={`swiper-slide ${
                                            currentCategoryNo === c.categoryNo
                                                ? 'category__header__menu--active'
                                                : ''
                                        }`}
                                        key={`sub-category-${c.categoryNo}`}
