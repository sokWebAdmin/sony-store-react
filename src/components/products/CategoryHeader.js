...
export default function CategoryHeader({ category, changeCurrentCategoryByNo }) {
  ...
  return (
    <div
      ...
      className={category?.depth > 1 ? 'category__header category__header__sub' : 'category__header '}
    ...
    >
        ...
        <div className="category__header__menu swiper-container">
          ...
          >
            {category?.children.length > 0 && (
              <SwiperSlide key="all" className="swiper-slide all category__header__menu--active">
                <a>
                  <span>전체보기</span>
                </a>
              </SwiperSlide>
      ...
      {rootParent && (
        <div className="category__header__links">
          ...
          {rootParent.isAvailableMoveAccessoryCompatibility && categoryLabel !== 'audio' && (
            <a
              href="#"
              className="category__header__link"
              onClick={(e) => {
            ...
            >
              <SwiperSlide
                className={`swiper-slide all ${
                  currentCategoryNo === category.categoryNo ? 'category__header__menu--active' : ''
                }`}
              >
                <a
                  href="#"
              ...
              {category?.children.map((c) => {
                return (
                  <SwiperSlide
                    className={`swiper-slide ${
                      currentCategoryNo === c.categoryNo ? 'category__header__menu--active' : ''
                    }`}
                    key={`sub-category-${c.categoryNo}`}
                  >
                    <a
