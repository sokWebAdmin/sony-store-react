...
export default function Product({ product, category, reset, micro }) {
    ...
    const _initGroupProducts = () => {
        ...
        const productNos = newGroupProducts
            .map(({ productNo }) => productNo)
            .filter(Boolean);
        productNos.length > 0 &&
            fetchProductGroupOptions(
                newGroupProducts.map(({ productNo }) => productNo),
            );
    };
    ...
    const fetchProductGroupOptions = async (productNos) => {
        ...
        data?.optionInfos &&
            setOptions(() =>
                data.optionInfos.flatMap(({ options }) => options),
            );
    };
    ...
    useEffect(() => {
        ...
        if (!groupManagementCode) {
            ...
            if (options?.length > 1) {
                setSaleStatus(
                    getSaleStatusForOption(options, product?.reservationData),
                );
            }
        }
    }, [product, options]);
    ...
    const _move = (product) => {
        const isEspCategory = !!getCategoryByKey(
            [espCategory],
            'categoryNo',
            Number(product.displayCategoryNos),
        );
        if (isEspCategory) {
            history.push('/esp');
        } else {
    ...
    return (
        ...
        >
            {product?.stickerLabels?.length > 0 &&
                tagColorMap[product.stickerLabels[0]] && (
                    <span
                        className='badge__text'
                        style={{ color: tagColorMap[product.stickerLabels[0]] }}
                    >
            ...
            <div
                ...
                style={micro ? { height: 'auto', paddingBottom: '100%' } : null}
            >
                <Link
                    onClick={(e) => {
                        e.preventDefault();
                    ...
                    {groupProducts.map((gp, index) => {
                        return (
                            <img
                                ...
                                className={`product__pic__img ${
                                    colorIndex === index &&
                                    'product__pic__img--visible'
                                }`}
            ...
            >
                <strong className='product__title__name'>
                    {product.productName}
                </strong>
                {saleStatus === 'RESERVE' && (
                    <span className='badge__label badge__label__reserve'>
                        예약판매
                    </span>
                )}
                {saleStatus === 'READY' && (
                    <span className={`badge__label badge__label__outofstock`}>
                        일시품절
                    </span>
                )}
                {saleStatus === 'SOLDOUT' && (
                    <span className={`badge__label badge__label__soldout`}>
                        품절
                    </span>
                )}
                {saleStatus === 'READY_RESERVE' && (
                    <span className={`badge__label badge__label__release`}>
                        출시예정
                    </span>
                )}
            </Link>
