...
export default function Floating({ location, scrollAction }) {
  ...
  const { scrollY, handleTop } = useScroll();
  ...
  return (
    <nav ref={sidebarRef} className={`sidebar ${active && 'sidebar--active'} sidebar--visible`} style={scrollStyle}>
      <div className="sidebar__inner">
        ...
        <a
          href={isMobile ? `tel:${TEL}` : '#none'}
          onClick={(e) => handleClick(e, 'cs')}
          className="sidebar__btn sidebar__btn__link customer"
        >
          <span>고객 센터</span>
        </a>
        <a href="#header" onClick={(e) => handleClick(e, 'top')} className="sidebar__btn top">
          <span>페이지 상단</span>
        </a>
      </div>
      <button
        onClick={() => (active ? toggle(false) : toggle())}
        className="sidebar__btn sidebar__btn__toggle"
        type="button"
      >
        <img src="../../images/common/ic_sidebar1.svg" alt="메뉴토글" />
      </button>
